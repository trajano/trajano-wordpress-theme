...
function twp_get_css_cdn($path)
{
    if ($path == get_template_directory_uri() . "/bootstrap/bootstrap.css") {
        return "//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.2/css/bootstrap-combined.no-icons.min.css";
        ...
        return sprintf("//netdna.bootstrapcdn.com/bootswatch/2.3.2/%s/bootstrap.min.css", $name);
    }
    return $path;
}
...
function twp_enqueue_style()
{
    if (!is_admin()) {
        ...
        if (get_theme_mod("trajano_use_cdn")) {
            ...
            wp_enqueue_style("bootstrap-responsive", "//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.2/css/bootstrap-responsive.min.css");
        } else {
...
add_action("wp_enqueue_scripts", "twp_enqueue_style");
